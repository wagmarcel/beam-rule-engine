...
class VcapServices(Vcap):
    ...
    def __init__(self):
        ...
        self.__parse_spark_credentials()
    ...
    def __parse_dashboard_url(self, ups):
        ...
        self.dashboard_url_normalized_for_spark = self.dashboard_url \
            .replace("http://", "") \
            .replace("https://", "") \
            .replace("-", "_") \
            .replace(".", "_")
    ...
    def __parse_spark_credentials(self):
        self.spark_credentials = self.json['gearpump'][0]['credentials']
        self.spark_url = self.spark_credentials['dashboardUrl']
        if 'username' in self.spark_credentials:
            self.spark_password = self.spark_credentials['password']
            self.spark_username = self.spark_credentials['username']
